...
class Route {
  ...
  toString() {
    return `<url><loc>${url}/${this.getPath()}</loc><lastmod>${`${new Date().getFullYear()}/${new Date().getDate()}/${("0" + (new Date().getMonth() + 1)).slice(-2)}`}</lastmod><changefreq>monthly</changefreq><priority>0.5</priority></url>`
  }
}
